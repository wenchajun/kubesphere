...
import (
	...
	"kubesphere.io/kubesphere/pkg/simple/client/os"
...
func NewClient(options *oslog.Options) (oslog.Client, error) {

	c := &client{}

	var err error
	c.c, err = os.NewClient(options.Host, options.BasicAuth, options.Username, options.Password, options.IndexPrefix, options.Version)
	return c, err
}

func (c *client) GetCurrentStats(sf oslog.SearchFilter) (oslog.Statistics, error) {
	var err error
	...
	if err != nil {
		return oslog.Statistics{}, err
	}

	return oslog.Statistics{
...
func (c *client) CountLogsByInterval(sf oslog.SearchFilter, interval string) (oslog.Histogram, error) {
	...
	for _, bucket := range resp.Buckets {
		h.Buckets = append(h.Buckets, oslog.Bucket{
			Time:  bucket.Key,
			Count: bucket.Count,
		})
	}
...
func (c *client) SearchLogs(sf oslog.SearchFilter, f, s int64, o string) (oslog.Logs, error) {
	...
	for _, hit := range resp.AllHits {
		...
		l.Records = append(l.Records, oslog.Record{
			Log:       s.Log,
			Time:      s.Time,
...
func (c *client) ExportLogs(sf oslog.SearchFilter, w io.Writer) error {
...
func parseToQueryPart(sf oslog.SearchFilter) *query.Query {
